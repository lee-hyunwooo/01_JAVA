/*
 ...
 public : 접근제한자 ( public, protected, private, default)
     ...
     private : 클래서 내에서만 접근 가능
     default : 같은 패키지 안에서 가능
     
      public을 쓰는 이유 : main은 프로그램에서 기본이기 때문에 public을 사용
      모든 클래스에서 접근 가능한 메소드임을 선언하고 시작하는 것.
     메소드(method) : 어떤 특정 작업을 수행하기 위한 명령문의 집합
     
     static / heap : 변수와 메소드를 메모리에 할당할 떄, 저장되는 영역
        static : 프로그램이 실행될 떄 저장.GC(Garbage Collector)가 정리하지 못함
        heap : 메소드에서 실행될 때 저장. GC(Garbage Collector)가 정리 가능
      main에서 static을 사용하는 이유는 프로그램의 기본이 되어야하는 함수가 정리되면 안되기 때문에
      static 영역에 메미로리 할당을 시켜주는 것
